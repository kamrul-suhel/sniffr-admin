...
trait VideoHelper{
    ...
    public static function getVideoHTML($video, $embed = false, $path = 'view') {
        ...
        $sHTML .= '<div class="'.($video->file || $video->youtube_id ? 'video-container' : '').(  $embed && !isset($video->youtube_id) ? ' embedded' : '').'" data-id="'.$video->alpha_id.'">';
        ...
        if($video->file || $video->file_watermark_dirty) {
            ...
            if(str_contains($video->url, 'posts') && $embed){
                $sHTML .= '<div class="interactive interactive-fb-post" style="background:#000 !important;text-align:center;"><div id="fb-root"></div><div class="fb-post" data-href="'.$video->url.'" data-width="165"></div></div>';
    ...
    public static function videoLinkChecker($url, $state = 'new'){
        ...
        $linkVars['image'] = $linkVars['image'] ? $linkVars['image'] : '/assets/images/placeholder.png';
        $linkVars['thumb'] = $linkVars['thumb'] ? $linkVars['thumb'] : '/assets/images/placeholder.png';
