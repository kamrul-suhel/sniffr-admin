...
trait VideoHelper{
    ...
    public static function getVideoHTML($video, $embed = false, $path = 'view') {
        ...
        if($video->youtube_id){ // Youtube
            ...
            if($video->file_watermark_dirty) {
                $file = self::generateCdn($video->file_watermark_dirty);
            } else{
                $file = $video->file;
            }

            $sHTML .= '<video id="video_player" x-webkit-airplay=”allow” class="video-js vjs-default-skin vjs-big-play-centered" controls preload="auto" poster="'.self::generateCdn($video->file_watermark_dirty).'" data-setup="{}" width="100%" style="width:100%;">
            ...
            $sHTML .= '<video 
                ...
                poster="';
            $sHTML .= self::generateCdn($video->image).'"';

           $sHTML .= 'src="'.self::generateCdn($video->file_watermark_dirty).'"></video>';
    ...
    public static function generateCdn($link)
    {
        return "https://cdn.sniffrmedia.co.uk/".basename($link);
    }
}
