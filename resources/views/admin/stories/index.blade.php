							...
							<div class="album-images-count">
                                <div class="album-info-extra">
									...
									@elseif($story->contacted_at && !$story->contact_made)
										...
										<strong>@if($story->reminders) {{ $story->reminders }} Reminder{{ ($story->reminders>1 ? 's' : '') }} : @else Contacted: @endif</strong>
										{{ (isset($story->contacted_at) ? \Carbon\Carbon::createFromFormat('Y-m-d H:i:s',$story->contacted_at)->diffForHumans() : 'Not yet') }}
										<a href="{{ url('admin/stories/reminder/'.$story->alpha_id.'/?decision='.$decision) }}" class="text-danger">{{ isset($story->contact->email) ? 'Remind' : 'Manually' }}</a>
									@else
