    ...
    <div class="album-images-count">
        <div class="album-info-extra">
            ...
            @if(isset($asset->contact))
                @if($asset->currentContract && $asset->contact->email)
                    <i class="fa fa-file-text"></i>
                    <strong>Contract {{ $asset->currentContract->reminders ? 'Re' : '' }}Sent:</strong>
                    <a href="#" class="btn-mini">{{\Carbon\Carbon::createFromFormat('Y-m-d H:i:s',$asset->currentContract->sent_at)->diffForHumans() }}</a>
                @elseif($asset->contacted_at && $asset->contact_made)
                    <i class="fa fa-check-circle-o" title="Made Contact"></i>
                    <strong>Made Contact:</strong>
                    <a href="#" class="btn-mini">{{ \Carbon\Carbon::createFromFormat('Y-m-d H:i:s',$asset->contacted_at)->diffForHumans() }}</a> <a href="{{ url('admin/stories/reminder/'.$asset->alpha_id) }}" class="text-danger btn-mini">Send {!! str_ordinal($asset->reminders + 1 , true) !!} Bump</a>
                @elseif($asset->contacted_at && !$asset->contact_made)
            ...
            @else
                ...
                <strong>No Contact or Unsubscribed</strong>
            @endif
        </div>
    </div>

    <div class="album-options no-border">
        @if(isset($chosen_decision))
            @foreach(config('stories.decisions.'.$chosen_decision) as $key => $state_values)
                @if($chosen_state == $key)
                    ...
                    @if($state_values['positive_label'])<a href="{{ ($asset->state=='licensing' ? url('admin/stories/edit/'.$asset->alpha_id.'/?decision='.lcfirst($chosen_decision)) : '#') }}" data-id="{{ $asset->alpha_id }}" class="{{ $state_values['positive_class'] }} btn-mini btn-mini-border" title="{{ $state_values['positive_label'] }}"><i class="fa fa-check"></i> {{ $state_values['positive_label'] }}</a> @endif
                @endif
            @endforeach
        @endif
    </div>
