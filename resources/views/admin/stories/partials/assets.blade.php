
<div class="panel panel-primary" data-collapsed="0">
    <div class="panel-heading">
        <div class="panel-title">Story Assets</div>

        <div class="panel-options">
            <a href="#" data-rel="collapse"><i class="fa fa-angle-down"></i></a>
        </div>
    </div>

    <div class="panel-body" style="display: block;">
        <div id="story_image_source" style="background-image:url('{{ (!empty($asset->thumb) ? $asset->thumb : '/assets/images/placeholder.png') }}');">
            <div class="story_image_asset_icon">
                <div style="display:inline-block;width:25px;height:25px;border-radius:4px;background-color:#fff;background-repeat:no-repeat;background-size: contain;background-image:url(data:image/png;base64,);"></div>
            </div>
        </div>
        @if(isset($asset) && $asset->assets()->count()>0)
        <div class="row">
            @foreach($asset->assets()->get() as $storyAsset)
                <div class="col-md-4">
                    <a id="story_image_asset_{{ $storyAsset->alpha_id }}" title="{{ $storyAsset->alpha_id }}" class="js-story-show-asset" href="{{ (!empty($storyAsset->thumbnail) ? $storyAsset->thumbnail : ($storyAsset->url ? $storyAsset->url : '/assets/images/placeholder.png')) }}">
                        <div class="story_image_asset" style="background-image:url('{{ (!empty($storyAsset->thumbnail) ? $storyAsset->thumbnail : ($storyAsset->url ? $storyAsset->url : '/assets/images/placeholder.png')) }}');">
                            <div class="story_image_asset_icon">
                                @if(!$storyAsset->jw_player_code)
                                <div style="display:inline-block;width:25px;height:25px;border-radius:4px;background-color:#fff;background-repeat:no-repeat;background-size: contain;background-image:url(data:image/png;base64,);"></div>
                                @else
                                <div style="display:inline-block;width:25px;height:25px;border-radius:4px;background-color:#fff;background-repeat:no-repeat;background-size: contain;background-image:url(data:image/png;base64,);"></div>
                                @endif
                            </div>
                        </div>
                    </a>
                </div>
            @endforeach
        </div>
        <hr />
        @endif
        <span class="input-group">
            <span class="input-group-addon">
                Upload Image
            </span>
            <input type="text" class="form-control" id="story_image_source_url" name="story_image_source_url" placeholder="" value="" />
            <input type="file" multiple="true" class="form-control" name="story_image" id="story_image"/>
        </span>
    </div>
</div>

<!-- Modal -->
<div class="modal fade" id="story_asset_modal" tabindex="-1" role="dialog" aria-labelledby="story_asset_modal_label" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-inner-content">
                <button class="btn" data-dismiss="modal"><i class="fa fa-times"></i> Close</button>
                <button class="btn btn-primary pull-right js-story-set-asset"><i class="fa fa-check"></i> Set Featured Image</button>
                <input type="hidden" id="story_asset_modal_set_featured" name="story_asset_modal_set_featured" value="" />
            </div>
        </div>
    </div>
</div>
