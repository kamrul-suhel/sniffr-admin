...
<div class="panel panel-primary" data-collapsed="0">
    ...
    <div class="panel-body" style="display: block;">
        <div id="story_image_source" style="background-image:url('{{ (!empty($asset->thumb) ? $asset->thumb : '/assets/frontend/images/placeholder.png') }}');">
        ...
        @if(isset($asset) && $asset->assets()->count()>0)
        <div class="row">
            @foreach($asset->assets()->get() as $storyAsset)
                <div class="col-md-4">
                    <a id="story_image_asset_{{ $storyAsset->alpha_id }}" title="{{ $storyAsset->alpha_id }}" class="js-story-show-asset" href="{{ (!empty($storyAsset->thumbnail) ? $storyAsset->thumbnail : ($storyAsset->url ? $storyAsset->url : '/assets/frontend/images/placeholder.png')) }}">
                        <div class="story_image_asset" style="background-image:url('{{ (!empty($storyAsset->thumbnail) ? $storyAsset->thumbnail : ($storyAsset->url ? $storyAsset->url : '/assets/frontend/images/placeholder.png')) }}');">
                            <div class="story_image_asset_icon">
                                @if(!$storyAsset->jw_player_code)
                                <div style="display:inline-block;width:25px;height:25px;border-radius:4px;background-color:#fff;background-repeat:no-repeat;background-size: contain;background-image:url(data:image/png;base64,);"></div>
                                @else
