<div class="panel panel-primary" data-collapsed="0">
    ...
    <div class="panel-body" style="display: block;">
        ...
        @include('admin.contacts.partials.select')
         ...
         <span class="form-group input-group">
            <span class="input-group-addon">
                Story type (dictates bump messages)
            </span>

            <select name="type" id="v" class="form-control js-problem-status">
                @foreach(config('stories.bump_type') as $key => $bump)
                <option value="{{ $key }}" {{ (isset($asset)  &&  $asset->type == $key) ? 'selected' : '' }}>{{ $bump['name'] }}</option>
                @endforeach
            </select>
        </span>
