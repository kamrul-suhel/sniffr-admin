...
@section('content')
	...
	<table class="table table-striped pages-table">
		<tr class="table-header">
			...
			<tr>
				<td>{{ TextHelper::shorten($mailer->stories()->where('story_id', $download->story_id)->first()->title, 150) }}</td>
				<td>{{ $users->where('id', $download->user_id)->pluck('email')->first() }}</td>
				<td>{{ $users->where('id', $download->user_id)->first()->client()->name }}</td>
				<td>
					@if($download!='video')
