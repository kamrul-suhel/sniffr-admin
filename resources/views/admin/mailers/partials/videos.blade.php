@foreach($mailer->videos as $video)
    <tr>
        <td width="60%">
            <h4>{{ TextHelper::shorten($video['title'], 250) }}</h4>
            <img src="{{ $video['image'] ? $video['image'] : $video['thumb'] ? $video['thumb'] : '/assets/frontend/images/placeholder.png' }}" border="0" style="display: flex; height: 200px; width: auto; margin-top: 15px;" />
            <br />
            <div style="display:inline-block;">
                <div style="display:inline-block;">
                    <div style="display:inline-block;width:25px;height:25px;background-repeat:no-repeat;background-size: contain;background-image:url(data:image/png;base64,);"></div>
                    <div style="display:inline-block;width:25px;height:25px;">1
                </div>
            </div>
        </td>
        <td>
            <br />{{ TextHelper::shorten($video['description'], 350) }}
            <br /><br /><a href="#" style="background:#000;color:#fff;padding:10px 6px;border-radius:5px;display:inherit;margin-top:20px;font-weight:bold;">View Video</a>
        </td>
    </tr>
@endforeach
